...
class db_mysql
{
   ...
   *
   *  @param str $dbuser, $dbpassword, $dbname, $dbhost, $dbcharset
   ****************************************************************************/

  function db_mysql($dbuser, $dbpassword, $dbname, $dbhost, $dbcharset="utf8")
  {
    $this->dbconn = mysql_connect($dbhost, $dbuser, $dbpassword);

    if (!$this->dbconn)
      $this->print_error("err : db connect err");

    mysql_query(" SET NAMES ".$dbcharset." ", $this->dbconn );
    mysql_query(" SET CHARACTER_SET_CLIENT = ".$dbcharset." ", $this->dbconn );
    mysql_query(" SET CHARACTER_SET_RESULTS = ".$dbcharset." ", $this->dbconn );
  ...
  function selectDB($dbname)
  {
    if ( !@mysql_select_db($dbname, $this->dbconn) )
      $this->print_error("select db error");
  }
  ...
  {
    $return_val = 0;                               // initialise return
    ...
    // -- Query was an select --------------------------------------------------
      ...
      $this->num_rows = $num_rows;    // Log number of rows the query returned
      $return_val = $this->num_rows;  // Return number of rows selected
      // -----------------------------------------------------------------------
